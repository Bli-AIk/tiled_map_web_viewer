...
pub fn run(config: ViewerConfig) {
    ...
    install_web_loading_overlay(&initial_loading_label, 0.12);
...
impl WebLoadingOverlayState {
    fn new(status_text: String) -> Self {
        Self {
            visible: true,
            status_text,
            progress: 0.12,
...
fn sync_web_loading_overlay(mut overlay: ResMut<WebLoadingOverlayState>) {
    if !overlay.dirty {
        return;
    }
